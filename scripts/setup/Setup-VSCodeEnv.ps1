...
function Get-VSCodePortable {
    ...
    if (Test-Path $exe) {
        ...
        elseif ($instalada) {
            ...
            Write-Log "    .\bin\setup\Setup-VSCodeEnv.bat -Force -KeepData" "WARN"
            return $vscodePath
        }
    }
...
foreach ($linea in @(Sync-VSCodeJavaRuntimes -Inicializar)) {
    ...
    if (@(Get-VSCodeExtensions -SettingsPath $settings) -notcontains 'redhat.java') {
        ...
        Write-Log "    .\bin\env\Use-VSCodeJava.bat -InstallExtension" "WARN"
    }
}
...
Write-Host "  Abrelo:       .\bin\start\Start-VSCodeEnv.bat" -ForegroundColor White
