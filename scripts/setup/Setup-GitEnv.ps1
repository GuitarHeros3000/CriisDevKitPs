...
function Get-GitPortable {
    ...
    if (Test-Path $gitExe) {
        ...
        elseif ($instalada) {
            ...
            Write-Log "  Para actualizarlo:  .\bin\setup\Setup-GitEnv.bat -GitVersion $($Release.Version) -Force" "WARN"
            return $gitPath
        }
        else {
...
if (-not $release) {
    ...
    if ([string]::IsNullOrWhiteSpace($GitVersion)) {
        ...
        Write-Log "    .\bin\setup\Setup-GitEnv.bat -GitVersion 2.55.0.5" "WARN"
    }
    else {
