...
function Get-GradlePortable {
    ...
    if (Test-Path $gradleBat) {
        ...
        elseif ($instalada) {
            ...
            Write-Log "  Para actualizarlo:  .\bin\setup\Setup-GradleEnv.bat -GradleVersion $($Release.Version) -Force" "WARN"
            return $gradlePath
        }
        else {
...
if (-not $release) {
    ...
    Write-Log "    .\bin\setup\Setup-GradleEnv.bat -GradleVersion 9.7.1" "WARN"
    exit 1
}
