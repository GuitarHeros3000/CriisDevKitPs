            ...
            if ($winner.Scope -eq 'maquina') {
                ...
                foreach ($k in $kitOnPath) {
                    ...
                    Add-Fix -Description "activar $($rt.Nombre) $($rt.Version) con Use-Env (toca tu perfil de PowerShell y el AutoRun de cmd)" `
                            -TocaPerfil `
                            -Arguments @{ Runtime = $rt.Runtime; Version = $rt.Version; Script = (Resolve-KitScript -Nombre 'Use-Env.ps1') } `
                            -Action {
...
function Test-KitIntegrity {
    ...
    $expected = @(
        ...
        "lib\Catalog.ps1",
        "lib\UserPath.ps1",
        "scripts\setup\Setup-AngularEnv.ps1",
        "scripts\setup\Setup-PythonEnv.ps1",
        "scripts\start\Start-AngularEnv.ps1",
        "scripts\start\Start-PythonEnv.ps1",
        "scripts\kit\Install-NoAdmin.ps1",
        "scripts\kit\Doctor-Env.ps1",
        "scripts\env\Uninstall-Env.ps1",
