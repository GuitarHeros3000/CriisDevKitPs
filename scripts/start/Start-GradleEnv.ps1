...
if ($versions.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "Ejecuta: .\bin\setup\Setup-GradleEnv.bat" -ForegroundColor White
    Write-Host ""
    exit 1
}
...
if (-not (Get-KitJavaHome)) {
    ...
    Write-Host "  Instalalo con .\bin\setup\Setup-JavaEnv.bat y reejecuta .\bin\setup\Setup-GradleEnv.bat" -ForegroundColor Gray
}
